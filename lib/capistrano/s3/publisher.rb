    ...
    module Publisher
          ...
          response = cf.create_invalidation({
            ...
            :invalidation_batch => {
              :paths => {
                ...
                :items => invalidations.map { |i| target_path + i }.map{ |i| if i.start_with?('/') then i else '/' + i end }
              },
      ...
      private
        ...
        def self.put_object(s3, bucket, target_path, path, file, only_gzip, extra_options)
          target_path += '/' unless target_path.end_with?('/') # Add trailing slash

          base_name = File.basename(file)
          mime_type = mime_type_for_file(base_name)
          options   = {
            :bucket => bucket,
            :key    => target_path + path,
            :body   => open(file),
            :acl    => 'public-read',
          }
          ...
          if mime_type
            ...
            if mime_type.sub_type == "gzip"
              ...
              options.merge!(key: target_path + self.orig_name(path)) if only_gzip
            end
          end
